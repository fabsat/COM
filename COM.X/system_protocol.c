...
static packet_format_t send_packet = PACKET_INIT;     // 送信データ用パケット データが送信されたら初期化される
static packet_format_t recv_packet = PACKET_INIT;     // 受信データ用パケット データ受信前に初期化される

/* 送信データ用パケットの格納できる先頭インデックスを示す データが送信されたら初期化される */
